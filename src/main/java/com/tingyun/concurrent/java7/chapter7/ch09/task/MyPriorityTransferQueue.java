package com.tingyun.concurrent.java7.chapter7.ch09.task;
...
/**
 * This class implements a priority based transfer queue.
 * It extends the
...
public class MyPriorityTransferQueue<E>
		extends PriorityBlockingQueue<E> implements TransferQueue<E> {
	...
	/**
	 * This method tries to transfer an element to a consumer.
	 * If there is
	...
	/**
	 * Transfer an element to the consumer.
	 * If there is a consumer waiting,
	 * puts the element on the queue and return the true value.
	 * Else, puts the
	 * value in the transfered queue and returns the false value.
	 * In this case, the
	 * thread than makes the call will be blocked
	 * until a consumer takes the transfered
	 * elements
	 */
	@Override
	...
	public E take() throws InterruptedException {
		...
		if (value==null) {
			lock.unlock();
			value=super.take();
			System.out.println("I am from priority queue");
			lock.lock();
		} else {
			synchronized (value) {
				value.notify();
